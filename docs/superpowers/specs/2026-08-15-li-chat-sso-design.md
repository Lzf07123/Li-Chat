...
`sso` 分支已按计划完成五个版本，全部通过 pytest/ruff/mypy，测试使用本地模拟 IdP（无外网依赖）：

| 版本 | 内容 | 状态 |
| --- | --- | --- |
| V1 | 项目骨架、配置、数据库、发现文档客户端 | ✅ 完成 |
| V2 | 授权码 + PKCE 登录闭环、令牌校验、用户落库 | ✅ 完成 |
| V3 | 本地会话、受保护路由、CSRF | ✅ 完成 |
| V4 | RP 登出、回程登出、WebSocket 桥接 | ✅ 完成 |
| V5 | 同源前端、真实发现文档校验、质量门禁 | ✅ 完成 |

真实 issuer 的连通性验证：发现文档与 JWKS 已实测可读，等 `client_id`/`client_secret` 注册完成后即可做端到端真机登录。
