...
指令补上页面特效层（极光 + 科技光效），流光线/卡片签名描边与 BlurText/CountUp 文字特效
不采用（无对应使用场景）。取舍记录见槽位 21/22 与设计规格
`docs/superpowers/specs/2026-08-17-design-template-v12-effects-adoption-design.md`。
...
| 22 | 按钮与光效风格 | 主按钮半透明单色着色（浅 `rgba(47,127,116,.10)` / 深 `rgba(127,212,198,.13)` + 同色细描边）+ `::after` 扫光 4s，文字浅 `#24433E` / 深 `#D7EFEA`；认证卡/Logo 呼吸辉光 4.5s；**极光层 + 科技光效层（网格/光束/光点）已采纳**（参数见 §4）。深色采用 D1 雾灰中间调；流光线与文字特效不采用 |
